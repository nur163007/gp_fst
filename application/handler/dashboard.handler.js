...
if(parseInt($("#currentRole").val())!==const_role_coupa_user) {
    ...
    if ($("#currentRole").val() == const_role_Corporate_Affairs) {
        window.location.href = "ca-interface";
    }
    if ($("#currentRole").val() == const_role_public_regulatory_affairs) {
        window.location.href = "pra-interface";
    }
    ...
    $(document).ready(function (e) {
        ...
        if ($("#currentRole").val() == const_role_Buyer) {
            ...
            $.get("api/purchaseorder?action=8", function (data) {
                if (!$.trim(data)) {
                    $("#myPendings").empty();
                    $("#myPendings").append(`<h4 class="well well-sm well-warning">No Pending</h4>`);
                } else {
                    //alert(data[0][0]);
