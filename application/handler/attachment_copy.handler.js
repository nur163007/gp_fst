    ...
    if(filter==""){
        for(var i=0; i<attach.length; i++){
            ...
            attachmentHtml += '<label class="col-sm-1 control-label"> </label>'+
                '<div class="col-sm-11">'+
                    '<label class="control-label"><i class="icon fa-'+attachments[i]['ext'].toLowerCase()+'"></i>  <a href="temp/'+attachments[i]['filename']+'" title="'+attachments[i]['filename']+'" target="_blank">'+attachments[i]['title']+'</a></label>'+
                '</div>';
        }
    }else{
        for(var i=0; i<attach.length; i++){
            if(filterType==1){
                if(filter.indexOf(attach[i]['title'])>-1){
                    if(attachments[i]['rolename']!=attachedBy){
                        if(attachedBy!=''){
                            if(col==2){
                                attachmentHtml += '</div>';
                ...
                if(filter.indexOf(attach[i]['title'])<0){
                    if(attachments[i]['rolename']!=attachedBy){
                        if(attachedBy!=''){
                            if(col==2){
                                attachmentHtml += '</div>';
