...
function _MakePWDWidget()
{
	...
	code += "<input type='password' class='form-control' name='"+pwdname+"' id='"+this.pwdfieldid+"'>";

	this.pwdtxtfield=pwdname+"_text";

	this.pwdtxtfieldid = this.pwdtxtfield+"_id";

	code += "<input type='text' class='form-control' name='"+this.pwdtxtfield+"' id='"+this.pwdtxtfieldid+"' style='display: none;'>";
	...
	this._showGeneratedPwd = showGeneratedPwd;

	this.generate_anch_obj = document.getElementById(this.pwdgenerate_anch);
	
	this.generate_anch_obj.pwdwidget=this;

	this.generate_anch_obj.onclick = function(){ this.pwdwidget._showGeneratedPwd(); };
...
function passwordStrength()
{
	...
	//Check upper case & lower case string. 1
	...
	//Check special chars. 2+1
	/*if ( password.match(/[^a-z\d]+/) )	{score++;}*/
	if ( password.match(/[!@#$%()^&*\-=+[\]{}/?.<>]/) )	{score++;}

	//Check string length. 3+1
