...
function generateReport($reportId){
    ...
    switch($reportId){
        case 1:
            ...
            (SELECT `name` FROM `wc_t_company` WHERE id = (SELECT `supplier` FROM `wc_t_po` WHERE poid = lc.`pono`)) AS `Supplier`,
            (SELECT `lcdesc` FROM `wc_t_po` WHERE poid = lc.`pono`) AS `Description of Goods`,
            (SELECT `name` FROM `wc_t_category` WHERE id = (SELECT `currency` FROM `wc_t_po` WHERE poid = lc.`pono`)) AS `Currency`,
            `lcvalue` AS `LC Value`,
        ...
        case 2:
            ...
            `pono` AS `PO No.`,
            (SELECT `name` FROM `wc_t_company` WHERE id = (SELECT `supplier` FROM `wc_t_po` WHERE poid = end.`pono`)) AS `Supplier`,
            (SELECT `lcdesc` FROM `wc_t_po` WHERE poid = end.`pono`) AS `Description`,
            (SELECT `name` FROM `wc_t_category` WHERE id = (SELECT `currency` FROM `wc_t_po` WHERE poid = end.`pono`)) AS `Currency`,
        ...
        case 3:
            $query = "SELECT 
                ...
                (SELECT `name` FROM `wc_t_company` WHERE id = (SELECT `supplier` FROM `wc_t_po` WHERE poid = amn.`poNo`)) AS `Supplier`,
               `amendDetail` AS `Description of Amendment`,
                (SELECT `name` FROM `wc_t_category` WHERE id = (SELECT `currency` FROM `wc_t_po` WHERE poid = amn.`poNo`)) AS `Currency`,
        ...
        case 4:
            $query = "SELECT 
            (SELECT co1.`name` FROM `wc_t_company` co1 WHERE id = (SELECT po1.`supplier` FROM `wc_t_po` po1 WHERE poid = lco.`pono`)) AS `Supplier`,
            (SELECT `name` FROM `wc_t_category` WHERE id = (SELECT po2.`currency` FROM `wc_t_po` po2 WHERE poid = lco.`pono`)) AS `Currency`,
            `lcvalue` AS `LC Value`,
            `lcvalue` AS `LC Value in USD`,
            'yyy' AS `LC Value in BDT`
        FROM `wc_t_lc` lco;";
