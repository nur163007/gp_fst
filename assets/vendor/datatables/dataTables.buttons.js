...
(function (d) {
    ...
    d.extend(l.prototype, {
        action: function (a, b) {
            ...
            a.children().detach();
            for (var c = 0,
                     e = b.length; c < e; c++)a.append(b[c].inserter), b[c].buttons && b[c].buttons.length && this._draw(b[c].collection, b[c].buttons)
    ...
    l.instanceSelector = function (a, b) {
        ...
        var c =
            ...
            if (d.isArray(a))for (var f = 0,
                                      r = a.length; f < r; f++)g(a[f]); else"string" === typeof a ? -1 !== a.indexOf(",") ? g(a.split(",")) : (a = d.inArray(d.trim(a), e), -1 !== a && c.push(b[a].inst)) : "number" === typeof a && c.push(b[a].inst)
        };
        g(a);
        return c
    };
    ...
    i.Api.registerPlural("buttons().containers()", "buttons().container()", function () {
        ...
        this.iterator(!0, "table", function (c) {
            if (c._buttons)for (var c = l.instanceSelector(b, c._buttons), d = 0,
                                    g = c.length; d < g; d++)a = a.add(c[d].container())
        });
        return a
    });
        ...
        if (this.context.length) {
            for (var b = new i.Api(this.context[0]),
                    ...
                    return c.format.footer(d ? d.innerHTML : "", a, d)
                     ...
                     h = b.cells(h, c.columns).nodes().toArray(), j = a.length, k = 0 < j ? f.length / j : 0,
                     l = Array(k), m = 0, n = 0; n < k; n++) {
