...
if (!empty($_GET["action"]) || isset($_GET["action"]))
{
    switch($_GET["action"])
    {
        ...
        case 16:
            ...
            break;
        case 17:
            echo getRequestData($_GET["refId"]);
            break;
...
function saveReqData()
{
    ...
    if($lastActionId==action_TAC_Approved_by_CPO) {
        ...
        if($action == 1) {
            ...
            //echo $query;
            $objdal->insert($query, "Could not save CAC-FAC info,");
        }
    }
...
function getDocTypevalue($po,$ciNo)
{
    ...
    $objdal = new dal();
    $query = "SELECT 
                p.*, c.`id` AS `partName`, c.`name` as `docName`, c.`tag` AS `docFullName`
            FROM 
            ...
            WHERE 
                ...
                AND `partname` NOT IN ( SELECT  p.`docName` FROM `wc_t_payment` p INNER JOIN `wc_t_lc` l ON p.`LcNo` = l.`lcno` 
                    WHERE l.`pono` = p.`pono` AND p.`ciNo` = '$ciNo') LIMIT 1;";
...
function getLetterText($poNo, $ship,$partName, $lastActId)
{
    ...
    if ($lastActId == action_TAC_Approved_by_PRUser || $lastActId == action_TAC_Reject_by_PRUser || $lastActId == action_TAC_Rejected_by_Buyer ||
        ...
        $query = "SELECT 
                  p.`lcbankaddress` AS `lcbeneficiary`,
                  c.`name` AS `suppName`
                  FROM `wc_t_po` p
        ...
        $objdal = new dal();
        $query = "SELECT `cfacLetterBody`, `certFinalApprover` FROM `wc_t_cacfac_request` 
...
function checkCPOApprove() {
    ...
    $query = "SELECT 
            ...
            FROM 
                `wc_t_action_log`
            WHERE 
                `PO` = '$poNo' AND shipNo='$ship' ORDER BY ID DESC;";
...
function getPayHistory($ciNo)
{
    ...
    $query = "SELECT 
                ...
                p.`paymentPercent`, 
                p.`amount`,
                DATE_FORMAT(p.`payDate`, '%M %d, %Y') AS `payDate`
            FROM 
                `wc_t_payment` p
...
function checkPamntStatus($poNo,$shipNo){
    ...
    /*if($actionStatus['paymentStatus']==1) {
        return 0;
    } else {
        return 1;
    }*/
...
function getRequestData($refId){
    $objdal = new dal();
    $refId = decryptId($refId);
    $sql = "SELECT `certReqId` FROM `wc_t_action_log` WHERE `ID` = $refId;";
    $reqId = $objdal->getScalar($sql);

    //Get certificate data
    $query = "SELECT `ciDesc`, `ciQty` FROM `wc_t_cacfac_request` WHERE `id` = $reqId;";
