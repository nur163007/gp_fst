...
function refreshNonSumReport() {
    ...
    if(!dtInit) {
        //alert("api/fn-report-lc-opening?action=1&lcno=" + lcno + "&bank=" + bank + "&supplier=" + supplier + "&cur=" + cur + "&start="+start+"&end="+end);
        $('#displayTable').DataTable({
            "ajax": "api/fn-report-lc-opening?action=1&lcno=" + lcno + "&pono=" + pono + "&bank=" + bank + "&supplier=" + supplier + "&cur=" + cur + "&start="+start+"&end="+end,
            "columns": [
                ...
                { "data" : "InsCoverNote" },
                { "data" : "Bank" },
                { "data" : "LC Opening Date" },
                { "data" : "Last Date of Shipment" },
                { "data" : "Expiry Date" },
                { "data" : "Supplier" },
                { "data" : "Description of Goods" },
                { "data" : "Currency" },
                { "data" : "LC Value", "class":"text-right" },
                { "data" : "LC Value in USD", "class":"text-right" },
                { "data" : "LC Value in BDT", "class":"text-right" },
                { "data" : "Insurance Value", "class":"text-right" },
                { "data" : "Payment Terms" },
                { "data" : "PONo" },
                { "data" : "PINo" },
        ...
        dtable.api().ajax.url("api/fn-report-lc-opening?action=1&lcno=" + lcno + "&pono=" + pono + "&bank=" + bank + "&supplier=" + supplier + "&cur=" + cur + "&start="+start+"&end="+end).load();
    }
}
...
function refreshSumReport(action) {
    ...
    if (!dtInitSum) {
        ...
        $('#displayTableSum').DataTable({
            ...
            "ajax": "api/fn-report-lc-opening?action=" + action + "&start=" + start + "&end=" + end,
            "columns": [
                {"data": "sumByColumn"},
        ...
        dtable.api().ajax.url("api/fn-report-lc-opening?action=" + action + "&start=" + start + "&end=" + end).load();
    }
}
